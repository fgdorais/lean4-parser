...
- `Stream.WellFounded` extends the `Std.Stream` class and provides an instance of `Stream.Finite`
  for every stream of the given type.
...
The `Stream.WellFounded` class is easiest to use: match on `Std.Stream.next?` and provide a
termination hint. The general pattern is this:
```
def foo [Stream.WellFounded σ α] (s : σ) : β :=
  match _hint : Std.Stream.next? s with
  | none => _
  | some (x, t) => _
termination_by s
```
...
```
def foo [Std.Stream σ α] (s : σ) [Stream.Finite s] : β :=
  match hint : Std.Stream.next? s with
  | none => _
  | some (x, t) =>
...
simple applications. The standard library already defines `Std.Stream` instances for productive pure
iterators. This module adds `Stream.WellFounded` instances for finite pure iterators. It also adds
instances the other way so that `StreamIterators` from well-founded streams translate to finite pure
iterators.
...
class WithNextRelation (σ : Type _) (α : outParam <| Type _) extends Std.Stream σ α where
  /-- Next relation for a stream type `σ`. -/
  rel : σ → σ → Prop -- this is named `rel` to match `WellFoundedRelation`
  /-- Next relation extends the default. -/
  rel_of_next?_eq_some : Std.Stream.next? s = some (x, t) → rel t s

/-- Default next relation for a stream type. -/
instance (priority := low) defaultNextRelation (σ) [Std.Stream σ α] : WithNextRelation σ α where
  rel t s := ∃ x, Std.Stream.next? s = some (x, t)
...
theorem next_of_next?_eq_some [WithNextRelation σ α] {s t : σ} :
    Std.Stream.next? s = some (x, t) → Next t s := WithNextRelation.rel_of_next?_eq_some
...
def WellFounded.ofRelation.{u,v} [Std.Stream.{u,v} σ α] [inst : WellFoundedRelation σ]
    (h : {s t : σ} → {x : α} → Std.Stream.next? s = (x, t) → WellFoundedRelation.rel t s) :
    Stream.WellFounded σ α := { inst with rel_of_next?_eq_some := h }

/-- Define a well-founded stream type instance from a measure. -/
def WellFounded.ofMeasure.{u,v} [Std.Stream.{u,v} σ α] (f : σ → Nat)
    (h : {s t : σ} → {x : α} → Std.Stream.next? s = (x, t) → f t < f s) :
    Stream.WellFounded σ α := { measure f with rel_of_next?_eq_some := h }

macro_rules
| `(tactic| decreasing_trivial) =>
  `(tactic| apply Stream.next_of_next?_eq_some; assumption)
...
theorem ofSome [WithNextRelation σ α] {s t : σ} [Stream.Finite s]
    (h : Std.Stream.next? s = some (x, t)) : Stream.Finite t := .ofNext <| next_of_next?_eq_some h
...
def foldlM [Monad m] [WithNextRelation σ α] (s : σ) [Stream.Finite s] (f : β → α → m β)
    (init : β) : m β :=
  match h : Std.Stream.next? s with
  | none => pure init
  | some (x, t) =>
...
theorem foldlM_none [Monad m] [WithNextRelation σ α] {s : σ} [Stream.Finite s]
    {f : β → α → m β} (h : Std.Stream.next? s = none) : foldlM s f init = pure init := by
  simp only [foldlM]
  split <;> simp_all

theorem foldlM_some [Monad m] [WithNextRelation σ α] {s t : σ} [Stream.Finite s]
    [Stream.Finite t]
    {f : β → α → m β} (h : Std.Stream.next? s = some (x, t)) :
    foldlM s f init = f init x >>= foldlM t f := by
  simp only [foldlM]
  split
  · simp_all
  · next heq =>
...
theorem foldrM_none [Monad m] [WithNextRelation σ α] {s : σ} [Stream.Finite s]
    {f : α → β → m β} (h : Std.Stream.next? s = none) : foldrM s f init = pure init := by
  rw [foldrM]; split <;> simp_all

theorem foldrM_some [Monad m] [WithNextRelation σ α] {s t : σ} [Stream.Finite s]
    [Stream.Finite t]
    {f : α → β → m β} (h : Std.Stream.next? s = some (x, t)) :
    foldrM s f init = foldrM t f init >>= f x := by
  rw [foldrM]; split <;> simp_all
...
theorem foldl_none [WithNextRelation σ α] {s : σ} [Stream.Finite s] {f : β → α → β}
    (h : Std.Stream.next? s = none) : foldl s f init = init := by
  rw [foldl]; split <;> simp_all

theorem foldl_some [WithNextRelation σ α] {s t : σ} [Stream.Finite s] [Stream.Finite t]
    {f : β → α → β} (h : Std.Stream.next? s = some (x, t)) :
    foldl s f init = foldl t f (f init x) := by
  rw [foldl]; split <;> simp_all
...
theorem foldr_none [WithNextRelation σ α] {s : σ} [Stream.Finite s]
    {f : α → β → β} (h : Std.Stream.next? s = none) : foldr s f init = init := by
  rw [foldr]
  split <;> simp_all

theorem foldr_some [WithNextRelation σ α] {s t : σ} [Stream.Finite s] [Stream.Finite t]
    {f : α → β → β} (h : Std.Stream.next? s = some (x, t)) :
    foldr s f init = f x (foldr t f init) := by
  rw [foldr]
  split <;> simp_all
...
theorem length_none [WithNextRelation σ α] {s : σ} [Stream.Finite s]
    (h : Std.Stream.next? s = none) : length s = 0 := foldl_none h
...
theorem length_some [WithNextRelation σ α] {s t : σ} [Stream.Finite s] [Stream.Finite t]
    (h : Std.Stream.next? s = some (x, t)) : length s = length t + 1 := by
...
theorem toListRev_none [WithNextRelation σ α] {s : σ} [Stream.Finite s]
    (h : Std.Stream.next? s = none) : toListRev s = [] := by
...
theorem toListRev_some [WithNextRelation σ α] {s t : σ} [Stream.Finite s] [Stream.Finite t]
    (h : Std.Stream.next? s = some (x, t)) : toListRev s = toListRev t ++ [x] := by
...
theorem toArray_none [WithNextRelation σ α] {s : σ} [Stream.Finite s]
    (h : Std.Stream.next? s = none) : toArray s = #[] := by
...
theorem toArray_some [WithNextRelation σ α] {s t : σ} [Stream.Finite s] [Stream.Finite t]
    (h : Std.Stream.next? s = some (x, t)) : toArray s = #[x] ++ toArray t := by
...
@[simp]
theorem toList_eq_self (l : List α) : toList l = l := by
  induction l with
